...
- **Decision:** **(b) Use reqwest directly.** Implemented as `AnthropicClient` in `core/semantic-engine/src/anthropic.rs`. reqwest 0.12 with JSON support was already a workspace dependency. The API surface is small (Messages API only), and this approach gives full control without depending on under-maintained community crates. If the client grows, it can be extracted to a separate crate later.
- **Resolved:** Phase 4 (2026-03-03)
